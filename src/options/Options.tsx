...
export function Options() {
  ...
  const exportData = async () => {
    ...
    anchor.href = url;
    anchor.download = "regroup.json";
    anchor.click();
    setTimeout(() => URL.revokeObjectURL(url), 0);
    setDataStatus({ text: copied ? "Copied and downloaded regroup.json" : "Downloaded regroup.json; clipboard unavailable" });
  };
  ...
  const importData = async () => {
    ...
    try {
      ...
      setDataStatus({ text: "Paste valid Regroup JSON.", error: true });
      return;
    }
    const window = await chrome.windows.getCurrent();
      ...
      <div className="mt-8 flex flex-col gap-6">
        ...
        <section className="flex flex-col gap-4" aria-labelledby="data-heading">
          ...
          <div>
            ...
            <p className="mt-2 text-xs text-muted-foreground">Copies JSON and downloads regroup.json.</p>
          </div>
          <div className="flex flex-col gap-2">
            ...
            <textarea
              ...
              placeholder="Paste Regroup JSON…"
