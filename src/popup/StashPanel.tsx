            ...
            {stashes.map((stash) => (
              <div key={stash.id} className="rounded-md border border-border/70 px-2 py-1.5">
                <div className="flex items-center gap-2">
                  ...
                  <IconButton title="Resume" disabled={disabled} onClick={() => onResume(stash.id)}>
                    {busyId === stash.id ? <LoaderCircle className="size-3.5 animate-spin" /> : <ArchiveRestore className="size-3.5" />}
                  </IconButton>
                  <IconButton title="Delete stash" disabled={disabled} onClick={() => onDelete(stash.id)}>
